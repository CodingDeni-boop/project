...
\begin{table}[h!]
\centering
\caption{Evaluation Metrics for the Model}
...
The confusion Matrix for Logistic Regression gave the following results:

\begin{figure}[h]
  \centering
  \includegraphics[width=0.6\textwidth]{confusion_matrix_Logistic_Regression.png}
  \caption{Confusion Matrix for the Logistic Regression classifier on the test dataset.}
  \label{fig:rf_confusion_matrix}
\end{figure}
...
\begin{table}[h!]
\centering
\caption{Evaluation Metrics for Logistic Regression on Test Data}
...
These metrics confirm that logistic regression did not just guess correctly by chance, but was able to learn meaningful patterns from the dataset.
...
To assess the performance of the KNN classifier, we analyzed the confusion matrix and computed key metrics.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.6\textwidth]{confusion_matrix_KNN.png}
  \caption{Confusion Matrix for the KNN classifier on the test dataset.}
  \label{fig:rf_confusion_matrix}
\end{figure}
...
\begin{table}[h!]
\centering
\caption{Evaluation Metrics with Class Breakdown}
...
The model performs well at detecting positive cases (class 1), with a very high recall of 96.5\%. This makes it suitable for applications where missing a positive case is costly.
